...
const filters: Array<{ value: ProcessFilter; label: string }> = [
  { value: 'all', label: 'All' }, { value: 'user', label: 'User' }, { value: 'system', label: 'System' }, { value: 'high-cpu', label: 'Higher CPU' }, { value: 'high-memory', label: 'Higher memory' }, { value: 'no-publisher', label: 'No publisher' }, { value: 'restricted', label: 'Restricted' },
]
...
export function ProcessesView({ processes, connections, currentUser }: { processes: ProcessInfo[]; connections: ConnectionInfo[]; currentUser?: string }) {
  ...
  const columns: OperationalColumn<ProcessInfo>[] = [
    ...
    { key: 'cpuPercent', label: 'CPU', width: '82px', render: (row) => <span className="metric-cell">{formatPercent(row.cpuPercent)}</span> },
    ...
    { key: 'publisher', label: 'Publisher', width: 'minmax(120px, 1.1fr)', priority: 'tertiary', render: (row) => displayValue(row.publisher) },
  ...
  return <>
    <section className="operational-panel">
      <OperationalToolbar query={query} onQueryChange={setQuery} filter={filter} onFilterChange={setFilter} options={filters} placeholder="Search process, PID, user, publisher, or path" meta={<><strong>{rows.length}</strong> of {processes.length}</>} />
    ...
    <Drawer open={Boolean(selected)} title="Process details" onClose={() => setSelectedKey(undefined)}>{selected && <div className="drawer-content">
      ...
      <DrawerSection icon={<Cpu size={15} />} title="Runtime"><Detail label="CPU" value={formatPercent(selected.cpuPercent)} /><Detail label="Memory" value={formatBytes(selected.memoryBytes)} /><Detail label="Threads" value={displayValue(selected.threadCount)} /><Detail label="Started" value={formatDateTime(selected.startTime)} /><Detail label="Uptime" value={formatDurationFrom(selected.startTime)} /></DrawerSection>
      <DrawerSection icon={<UserRound size={15} />} title="Ownership"><Detail label="User" value={displayValue(selected.user)} /><Detail label="Publisher" value={displayValue(selected.publisher)} /><Detail label="Signature" value={displayValue(selected.signatureStatus)} /><Detail label="Access" value={selected.accessStatus} /></DrawerSection>
