...
export function scoreAvailability(score: SecurityScore | null, t?: TFunction<'score'>) {
  ...
  if (score.state === 'limited') {
    ...
    return { available: false, title: t?.('state.limited') ?? 'Limited coverage', detail: translatedReason(score.reason, fallback, t) } as const
  }
  ...
  return { available: true, title: scoreLabel(score.label, t), detail, value: score.score } as const
}
